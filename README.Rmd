...
vcMatchFileNames = '/home/ask/Documents/open-data/data/events/69334.json'
...
fPlotSonar = function (
   ...
   # If the data isn't being divided into blocks based on the origin of the 
   # passes then assume all passes to be in the same block
   ...
   # Marking the data for different sonars based on the input coordinates
   dtPasses[,
      location1.bucket := ( 
         nBlockLength1 * ( location1 %/% nBlockLength1 ) 
      ) + ( nBlockLength1 / 2 )
   ]
   
   dtPasses[,
      location2.bucket := ( 
         nBlockLength2 * ( location2 %/% nBlockLength2 ) 
      ) + ( nBlockLength2 / 2 )
   ]
...
plotPitchBreakupSonar = fPlotSonar (
   dtPassesToPlot = lMatches[['69334']]$Pass,
   iBlocksInFirstRing = 4,
   iNbrRings = 8,
   nLocation2Size = 80,
   nLocation1Size = 120,
