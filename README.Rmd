...
- Each block in the ring captures passes of the respective lengths in a paritcular range of direction. The blocks have been calculated in such a way to cover approximately the same sized area of the pitch as any other block, which is why each block spans a lesser part of the circumference on passes of longer lengths compared to the part of the circumference it covers on passes of shorter lengths.
was.

- The thickness of each block in the ring is proportional to the number of passes of that length and angle. ( It's actuallly proportional to the square root of the number of passes, if I make it proportional then the blocks with few passes become too small to be visible. Suggestions to solve this would be highly appreciated. )
...
fPlotPitchBreakupSonar = function (
   ...
   # vnPassAngleQuantiles = quantile(dtPasses[, pass.angle], (0:iBlocksInFirstRing)/iBlocksInFirstRing),
   ...
   vnPassLengthBreaks = seq(0, nLocation1Size, nPassLengthBreakGap)
   nPassAngleBreakGap = 2 * pi / iBlocksInFirstRing
   ...
   vnCurrentLogicAreas = vnPiR2Difference / ( 4 * seq ( length ( ( head(vnPassLengthBreaks, -1) ) ) ) )
   ...
   dtPasses[, 
      pass.angle.bucket := pass.angle %/% ( 2 * pi / viNbrAngleBreaks[ 1 + pass.length.bucket ] ),
      pass.length.bucket
   ]
   
   dtPasses[
      pass.angle.bucket == pi %/% ( 2 * pi / viNbrAngleBreaks[ 1 + pass.length.bucket ] ), 
      pass.angle.bucket := pass.angle.bucket - 1
   ]
   
   
   dtPasses[, 
      pass.angle.bucket.width := ( 2 * pi / viNbrAngleBreaks[ 1 + pass.length.bucket ] ), 
      pass.length.bucket
   ]
